...
def count(request):
    if request.method == 'GET':
        ...
        # 统计排名(sql语句)
        sql_str = "SELECT thing.id, thing.title, COUNT(`order`.id) AS count " \
                  "FROM b_order AS `order` " \
                  "JOIN b_thing AS thing ON `order`.thing_id = thing.id " \
                  "WHERE `order`.status = '2' " \
                  "GROUP BY thing.id, thing.title;"
